function plot_pnas_8th_order_acc_gfa_diff

    % 10th Order
    %load('result_b3000_testing_10th_order.mat')
    %load('siam_check_1.mat')
    ...
    for i=1:dims(1)
        ...
        csd_gfa_vector(i,1) = calcGFA(csd_pred_temp);
    end
    
    diff_gfa_vector = true_gfa_vector - pred_gfa_vector;
    csd_diff_gfa_vector = true_gfa_vector - csd_gfa_vector;
    ...
    xlabel('GFA')
    ylim([0 450])
    title('Diff in GFA spread NSDN (True-Pred)')
    ...
    subplot(1,3,2)
    hist(csd_gfa_vector,100)
    grid on
    ylabel('Number of Voxels')
    xlabel('GFA')
    xlim([0.2 1])
    ylim([0 450])
    title('Predicted CSD GFA spread')
      
    subplot(1,3,3)
    hist(csd_diff_gfa_vector,100)
    grid on
    ylabel('Number of Voxels')
    xlabel('GFA')
    ylim([0 450])
    xlim([-0.3 0.6])
    title('PDiff in GFA spread CSD (True-CSD)')
